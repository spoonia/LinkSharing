...
class JqueryValidationService {
	...
	private String getMessage(Class validatableClass, String propertyName,
	                          def args, String constraintName, Locale locale) {
		...
		constraintNames.eachWithIndex { constraintName, i ->
			...
			if (javaScriptConstraint) {
				...
				if (customConstraintsMap && customConstraintsMap[constraintName]) {
					switch (constraintName) {
						case "equalsTo":
							javaScriptConstraintCode = "${constraintName}: " +
									"${constrainedProperty.propertyType == Date ? "new Date(${constrainedProperty.equalsTo.time})" : "'${constrainedProperty.equalsTo}'"}"
							break;
						default:
